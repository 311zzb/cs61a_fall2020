...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9                 # ((f x) (+ x 2))
        func_name   = target.first        #   f
        formals     = target.rest         #     x
        body        = expressions.rest    #        (+ x 2)
        lambda_form = Pair(formals, body) # (  (x) (+ x 2))
        func        = do_lambda_form(lambda_form, env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        else: # expressions.first is false
            return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    bindings_list = pair_to_list(bindings)
    for binding_pair in bindings_list:
        validate_form(binding_pair, 2, 2)
        name, value = binding_pair.first, binding_pair.rest.first
        value = scheme_eval(value, env) # values need to be value instead of expr
